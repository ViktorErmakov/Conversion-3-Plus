...
// Показать интерактивное окно выбора конвертации для правильно описания метаданных в консоле.
//
// Параметры:
//  Форма				 - ФормаКлиентскогоПриложения - форма источник вызова процедуры.
//  СписокКонвертаций	 - СписокЗначений - список конвертаций для которых используется обработчик.
//
//&НаКлиенте
...
//
&НаКлиенте
Процедура ПриНажатии(Форма, ДанныеСобытия) Экспорт
...
Процедура СоздатьКоманднуюПанель(ЭлементАлгоритма, Форма, Родитель, ЭлементПеред)
	...
	Справочник = СтрРазделить(Форма.ИмяФормы, ".")[1];
	Если Не Справочник = "Конвертации" Тогда
		СоздатьГруппуКонвертации(Форма, ЭлементАлгоритма, КоманднаяПанель);
	КонецЕсли;
	...
	Картинка = БиблиотекаКартинок.Переименовать;
...
&НаСервере
Процедура СоздатьГруппуКонвертации(Форма, ЭлементАлгоритма, КоманднаяПанель)
	
	//ЭлементыФормы = Форма.Элементы;
	//
	//// добавить группу команд выбор конвертации с выпадающим списком.
	//ЭлементГруппаКомандКонвертации = ЭлементыФормы.Вставить("ГруппаКонвертации", Тип("ГруппаФормы"), КоманднаяПанель);
	//ЭлементГруппаКомандКонвертации.Заголовок = "Конвертация";
	//ЭлементГруппаКомандКонвертации.Вид = ВидГруппыФормы.Подменю;
