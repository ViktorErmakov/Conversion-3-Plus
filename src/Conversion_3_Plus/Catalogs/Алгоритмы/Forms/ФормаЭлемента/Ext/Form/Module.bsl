...
Процедура ОбновитьОписаниеМетаданных()
	...
	ЭтотОбъект["ОписаниеМетаданныхJSON"] = ПолучитьОписаниеМетаданныхJSON(ЭтотОбъект["Конвертация"], Объект.Параметры);
	ОбновитьОписаниеМетаданныхВоВсехКонсоляхОбработчиков();
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьОписаниеМетаданныхJSON(Знач Конвертация, ПараметрыАлгоритма, Знач Направление = "")
	...
	Если Не ПустаяСтрока(ПараметрыАлгоритма) Тогда
		...
		Для Каждого Параметр Из СписокПараметров Цикл
			ПользовательскиеОбъекты.Вставить(Параметр, "");
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбновитьОписаниеМетаданныхПоНаправлению(ИмяКоманды)
	...
	ЭтотОбъект["ОписаниеМетаданныхJSON"] = ПолучитьОписаниеМетаданныхJSON(ЭтотОбъект["Конвертация"], Объект.Параметры, Направление);
