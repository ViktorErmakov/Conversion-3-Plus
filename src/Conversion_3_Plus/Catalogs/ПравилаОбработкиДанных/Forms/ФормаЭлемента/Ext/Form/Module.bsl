...
Процедура Расш1_СтраницыПриСменеСтраницыПосле(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "ОбработчикиСобытийXDTO" 
		Или ТекущаяСтраница.Имя = "Страница_ПриОбработке" 
		Или ТекущаяСтраница.Имя = "Страница_ВыборкаДанных" Тогда
		
		ИспользованиеПКО = Новый Структура;
		
		Если Элементы.СтраницыОбработчикиXDTO.ТекущаяСтраница.Имя = "Страница_ПриОбработке" И ИспользоватьНесколькоПравилКонвертации Тогда
			Для Каждого Строка Из Объект.ПравилаКонвертацииОбъектов Цикл
				ИспользованиеПКО.Вставить(Строка.ПравилоКонвертацииОбъекта, Истина);
			КонецЦикла;
		...
		ОбновитьОписаниеМетаданных(ИспользованиеПКО);
...
&НаКлиенте
Процедура ОбновитьОписаниеМетаданных(ИспользованиеПКО)
	...
	ЭтотОбъект["ОписаниеМетаданныхJSON"] = 
		ПолучитьОписаниеМетаданныхJSON(ЭтотОбъект["Конвертация"], Объект.ОбъектВыборки, Направление, ИспользованиеПКО);
	ОбновитьОписаниеМетаданныхВоВсехКонсоляхОбработчиков();
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьОписаниеМетаданныхJSON(Знач Конвертация, Знач ОбъектВыборки, Знач Направление, Знач ИспользованиеПКО = Неопределено)
	...
	Если ИспользованиеПКО <> Неопределено И 
		...
		ИспользованиеПКОJson = Справочники.Конфигурации.ПолучитьОписаниеИспользованиеПКО(ИспользованиеПКО);
